...
const ExpandableMarkdown = ({ content, expanded = false }: { content: string; expanded?: boolean }) => {
  ...
  const displayContent = shouldTruncate && !isExpanded ? paragraphs[0] + "..." : content;

  return (
    <span>
      <MarkdownRenderer content={displayContent} />{" "}
      {shouldTruncate && (
        <Button appearance="subtle" onClick={() => setIsExpanded(!isExpanded)} className={styles.toggleButton}>
          {isExpanded ? (
            <>
              less
              <ChevronUp16Filled />
...
const AgentMessage: React.FC<AgentMessageProps> = ({ agentStatus, currentTool, message, isWorking, parentContainerHeight, isLastMsg }) => {
  const styles = useStyles();
  const parentHalfHeight = parentContainerHeight ? parentContainerHeight / 2 : undefined;
  ...
  if (message.toolResults?.length) {
    ...
    try {
      ...
      if (typeof parsedContent.accumulatedOutput === "string" && Array.isArray(parsedContent.tool_results)) {
  ...
  const toolDataMap = useMemo(() => {
    return (tool_results || []).reduce((acc, result) => {
      try {
        const toolName = result.toolName;
        let parsedData;
        if (result.data && result.data[0] && typeof result.data[0].text === "string") {
        ...
        console.error(`Failed to parse tool result data for ${result.toolName}:`, e);
      }
      return acc;
  ...
  if (accumulatedOutput) {
    ...
    for (const match of matches) {
      ...
      if (textBefore) {
        contentElements.push(<MarkdownRenderer key={`text-${lastIndex}`} content={textBefore} />);
      }

      const toolName = match[1];
      if (toolRenderIndexMap[toolName] === undefined) toolRenderIndexMap[toolName] = 0;
      ...
      if (toolData) {
        switch (toolName) {
          ...
          case "query-tc-skills":
            contentElements.push(<SkillResultCard key={`tool-${toolName}-${match.index}`} skills={toolData} compMaxHeight={parentHalfHeight} />);
            break;
          default:
            contentElements.push(<GenericResultCard key={`tool-${toolName}-${match.index}`} data={toolData} />);
            break;
        }
  ...
  const renderStatusIndicator = () => {
    const getStatusText = (): string => {
      switch (agentStatus) {
        ...
        case "calling_tool":
          return currentTool ? `Calling Tool: ${currentTool}...` : "Calling a tool...";
    ...
    return (
      <div className={styles.spinnerContainer}>
        ...
        <Text style={{ fontSize: "13px", color: "var(--colorNeutralForeground3)" }}>{getStatusText()}</Text>
      </div>
    );
  };
